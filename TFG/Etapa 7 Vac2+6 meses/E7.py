...
pacientes_infectados = df[df.iloc[:, 93] == 1]  # Filtrar pacientes infectados con valor 1 en columna 43, PACIENTES INFECTADOS
pacientes_enfermos = df[df.iloc[:, 94] == 1]  # Filtrar pacientes enfermos con valor 1 en columna 44, PACIENTES ENFERMOS
...
# Obtener los valores de pacientes infectados por tipo de paciente
infectados_transplante = pacientes_infectados_tipo.get(1, 0)
infectados_dialisis_peritoneal = pacientes_infectados_tipo.get(2, 0)
infectados_hemodialisis = pacientes_infectados_tipo.get(3, 0)

# Obtener los valores de pacientes enfermos por tipo de paciente
enfermos_transplante = pacientes_enfermos_tipo.get(1, 0)
enfermos_dialisis_peritoneal = pacientes_enfermos_tipo.get(2, 0)
enfermos_hemodialisis = pacientes_enfermos_tipo.get(3, 0)

# Graficar la barra de pacientes infectados dividida por tipo de paciente
ax.bar(x[0], infectados_transplante, color=colores[0], label='Paciente con trasplante')
ax.bar(x[0], infectados_dialisis_peritoneal, bottom=infectados_transplante, color=colores[1], label='Paciente con Diálisis Peritoneal')
ax.bar(x[0], infectados_hemodialisis, bottom=infectados_transplante + infectados_dialisis_peritoneal, color=colores[2], label='Paciente con hemodiálisis')

# Graficar la barra de pacientes enfermos dividida por tipo de paciente
ax.bar(x[1], enfermos_transplante, color=colores[0])
ax.bar(x[1], enfermos_dialisis_peritoneal, bottom=enfermos_transplante, color=colores[1])
ax.bar(x[1], enfermos_hemodialisis, bottom=enfermos_transplante + enfermos_dialisis_peritoneal, color=colores[2])
...
pacientes_transplante_infectados = df[(df.iloc[:, 10] == 3) & (df.iloc[:, 94] == 1)]
...
media_anticuerpos_2040 = df.loc[(edades >= 20) & (edades <= 40)].iloc[:, 190].mean()
media_anticuerpos_4060 = df.loc[(edades > 40) & (edades <= 60)].iloc[:, 190].mean()
media_anticuerpos_6080 = df.loc[(edades > 60) & (edades <= 80)].iloc[:, 190].mean()
media_anticuerpos_80plus = df.loc[edades > 80].iloc[:, 190].mean()
...
plt.show()
#----------------------------------------------------Analisis de infeccion y enfermedad----------------------------------------------------------------------------------
pacientes_infectados = df[df.iloc[:, 93] == 1] 
pacientes_enfermos = df[df.iloc[:, 94] == 1]  
pacientes_infectados_tipo = pacientes_infectados.iloc[:, 10].value_counts().sort_index()
pacientes_enfermos_tipo = pacientes_enfermos.iloc[:, 10].value_counts().sort_index()
